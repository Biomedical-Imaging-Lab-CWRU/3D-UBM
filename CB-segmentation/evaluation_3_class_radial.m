...
for i= 1 : length(vol_names)
    ...
    imDir = fullfile(['..\data\cb_3_class_radial\images']);
    pxDir = fullfile(['..\data\cb_3_class_radial\gt']);
    ...
    for j= 1:length(pxds_va.Files)
        ...
        result = imresize(imread(fullfile(['..\data\cb_3_class_radial\results\' vol_names{i} '_' num2str(j) '.png'])),size(gt));
...
%         res_vol(:,:,j) = result2;
%         
%            [c_matrix,Result,RefereceResult]= confusion.getMatrix(gt,result2,0);
%         
%            scores(cnt) = Result.F1_score;
        gt_vol(1:size(gt,1),1:size(gt,2),ndx(j)) = gt;
        res_vol(1:size(gt,1),1:size(gt,2),ndx(j)) = result2;
        cnt = cnt +1;
...
1+(sum(res_vol_pre(:) == 2)-sum(gt_vol_pre(:) == 2))/sum(gt_vol_pre(:) == 2)

1+(sum(res_vol_post(:) == 1)-sum(gt_vol_post(:) == 1))/sum(gt_vol_post(:) == 1)
1+(sum(res_vol_post(:) == 2)-sum(gt_vol_post(:) == 2))/sum(gt_vol_post(:) == 2)

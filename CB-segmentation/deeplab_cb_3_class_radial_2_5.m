...
for i= 1 : length(vol_names)
    ...
    imDir = fullfile(['..\data\cb_3_class_radial_2.5D\images']);
    pxDir = fullfile(['..\data\cb_3_class_radial_2.5D\gt']);
    ...
    %
    tempdir = fullfile(['..\data\cb_3_class_radial_2.5D\results']);
    pxdsResults = semanticseg(pximds_va,Trained_Net,'MiniBatchSize',4,'WriteLocation',tempdir, 'NamePrefix', vol_names{i});
    
    
    save(['deeplab_cb_3_class_radial_dice_2.5D' vol_names{i} '.mat'], 'Trained_Net', 'info');
    
    save(['validation_cb_3_class_radial_dice_2.5D' vol_names{i} '.mat'], 'pximds_va');

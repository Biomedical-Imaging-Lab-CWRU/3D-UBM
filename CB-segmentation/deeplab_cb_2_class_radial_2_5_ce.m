...
for i= 1 : length(vol_names)
    ...
    imDir = fullfile(['..\data\cb_2_class_radial_2.5D\images']);
    pxDir = fullfile(['..\data\cb_2_class_radial_2.5D\gt']);
...
%         lgraph = connectLayers(lgraph,'softmax-out','diceLayer');
    ...
    %
    tempdir = fullfile(['..\data\cb_2_class_radial_2.5D\results_ce']);
    pxdsResults = semanticseg(pximds_va,Trained_Net,'MiniBatchSize',4,'WriteLocation',tempdir, 'NamePrefix', vol_names{i});
    
    
    save(['deeplab_cb_2_class_radial__2.5D_ce_' vol_names{i} '.mat'], 'Trained_Net', 'info');
    
    save(['validation_cb_2_class_radial_2.5D_ce_' vol_names{i} '.mat'], 'pximds_va');

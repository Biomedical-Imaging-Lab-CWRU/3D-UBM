...
for i= 1 : length(vol_names)
    ...
    %
    tempdir = fullfile(['..\data\cb_3_class\results']);
    pxdsResults = semanticseg(pximds_va,Trained_Net,'MiniBatchSize',4,'WriteLocation',tempdir, 'NamePrefix', vol_names{i});
    
    
    save(['deeplab_cb_3_class_ce_' vol_names{i} '.mat'], 'Trained_Net', 'info');
    
    save(['validation_cb_3_class_ce_' vol_names{i} '.mat'], 'pximds_va');
